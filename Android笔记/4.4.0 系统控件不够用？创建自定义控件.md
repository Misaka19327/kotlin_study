...
然后，在TitleLayout中的init构造体中加入按钮的逻辑。

![1667813777204](image/4.4.0系统控件不够用？创建自定义控件/1667813777204.png)

如果点击Back按钮则结束Activity，如果点击Edit按钮则弹出一段文字。因为TitleLayout接收的context参数实质上是一个Activity实例，需要用as转换为Activity类型，然后调用finish()销毁。

通过这种方法，就可以每次引入该控件时，都自带点击事件，从而节省代码。
