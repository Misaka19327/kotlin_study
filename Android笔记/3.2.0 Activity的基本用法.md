...
其中 main文件通过item指定具体的菜单项 android:id指定菜单项的唯一id android:title指定了菜单项的文字内容
...
这段代码几乎一目了然 只有when()这里 实际上是执行了getItenId()方法 和上面的getMenuInflater()方法原理相同 用于获取菜单内项目的id
...
销毁一个Activity有两种方法 第一种就是直接点返回键 就能够销毁当前Activity 如果想要使用Activity中的别的键达成销毁Activity的目的 就需要调用finish()方法 比如说我可以让Button 1按钮销毁当前Activity：

![1666941655454](image/3.2.0Activity的基本用法/1666941655454.png)

就这样就可以了 现在点击Button 1就可以直接返回桌面
