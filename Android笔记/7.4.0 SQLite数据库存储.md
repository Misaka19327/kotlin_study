...
这里的一个解决方法就是改为调用getColumnIndexOrThrow()（这个方法大概就是用try-catch把原方法封装一下，使其返回值从原来的-1变成0），或者加一个<@SuppressLint("Range")>声明使其忽略警告。
...
除了用上面那些Android提供的API来操作数据库，也可以用一种相当传统的方式来操作：即直接写SQL语句并通过SQLiteDatabase对象的的execSQL()来直接执行，当然，结果是完全相同的。上面的那些操作如果用SQL语句来操作需要这样写：
