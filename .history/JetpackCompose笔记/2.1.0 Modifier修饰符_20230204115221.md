...
以Layout这个Composable为例，假设说这个Modifier是给这个Layout组件的，那么根据源码，这个Modifier最终会被传到materializerOf方法中。而在这个方法里，调用了materialize()方法：

![1675482315904](image/2.1.0Modifier修饰符/1675482315904.png)

![1675482471607](image/2.1.0Modifier修饰符/1675482471607.png)

最终可以看到，materialize方法里出现了foldIn的调用，当Modifier链中包含ComposeModifier时，也会在这里被摊平，将Modifier(Factory)加入链中，最终链中将不再有ComposedModifier。Modifier链后续还会使用foldOut方法进行遍历，生成LayoutNodeWrapper链来间接影响组件的测量布局与渲染。
