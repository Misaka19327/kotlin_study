...
在传统View视图系统中，可以在XML文件中为View组件设置资源ID，并且将资源ID作为索引来声明组件应当摆放的位置。在Compose中的这个版本的ConstraintLayout里，显然是不需要通过资源ID来声明组件了，但是这个方法依旧适用于如何确定组件位置。在该布局中，可以主动创建引用并将其绑定在某个具体
