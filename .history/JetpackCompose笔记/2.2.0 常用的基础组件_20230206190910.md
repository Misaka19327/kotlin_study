...
这个上tag，依赖的是pushStringAnnotation()和pop()两个方法，tag的限定区域就是在这两个方法之间append进去的文本。但是单一个AnnotatedString组件是不够的，它只能显示文字，给文字加tag，真正实现点击的是ClickedText组件。在上面这个示例中，首先先把写好的AnnotatedString传进去，之后在onClick事件里写具体的逻辑。获取的annotation就是上面加tag时加进去的信息，
