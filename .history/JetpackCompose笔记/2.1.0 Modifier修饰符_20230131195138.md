...
border 一般用来为被修饰组件添加边框。这个边框可以指定颜色，粗细和形状。而 padding 一般被用来为被修饰组件增加间隙。可以在 border 的前后各插入一个 padding，区分对外和对内的间隙。相较于传统 View 视图有 Margin 和 Padding 之分，Compose 中只有 padding 这一种修饰符，根据不同情况和不同的位置
