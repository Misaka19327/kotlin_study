...
在Compose中，有两种创建引用的方式：createRef和createRefs，前者对应单个引用，后者对应多个引用。创建引用之后可以使用修饰符Modifier.constrainAs将其绑定在某个组件上，并且在尾部Lambda内指定组件的约束信息。但是，创建引用和绑定引用这两个只能在ConstraintLayout中进行，因为ConstrainScope尾部Lambda的Receiver是一个ConstraintLayoutScope作用域对象。
