...
类Button的组件并非唯一可以点击的组件类别。理论上，任何Composable组件都可以通过Modifier.clickable修饰符变成可点击的组件。但是Button组件在点击后还需要额外进行一些事件响应处理，比如说显示Material Design风格的水波纹的点击特效。这些都是其内部通过拦截Modifier.clickable事件实现的处理
