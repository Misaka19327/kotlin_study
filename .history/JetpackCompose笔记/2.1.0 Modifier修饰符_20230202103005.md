...
而在Compose的语法结构里，一般只需要调用当前作用域的方法，而像这段示例代码里这样跨级访问会加大报错的概率。在Compose中为了解决这个问题，可以使用@LayoutScopeMarker注解来规避Receiver的跨级访问。而Compose的常用组件都已经使用了这个注解。对于添加了这个注解的Receiver，其作用域中牧人只能调用作用域提供的方法
