...
对于handleSimpleData来说，它负责创建一个Teacher实例，然后把传进来的SimpleData实例里的Student实例替换掉，这段操作是没有问题的，反正它所需要的参数是 `SimpleData<Person>`，Teacher也是Person的子类，能够通过编译。

那么理论上，最后在main里get()出来的应该是这个Teacher的实例。但是这里要注意一点，
