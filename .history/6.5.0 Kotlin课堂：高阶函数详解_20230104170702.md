...
首先，在这个高阶函数里，创建了一个Runable对象，并且在它的Lambda表达式中调用了这个高阶函数传入的函数类型参数。而Lambda表达式在编译时会被编译为匿名类的实现方式，也就是说，上述代码实际上是在匿名类中调用了传入的函数类型参数。

又因为它是个内联函数，那么所引用的Lambda表达式就会允许使用return关键字进行函数返回。但是由于它是在匿名类中调用的函数类型参数，所以此时时不可能进行外层调用函数返回的，最多只能对匿名类中的函数调用进行返回，因此不能通过编译。

简而言之，如果高阶函数中创建了另外的Lambda表达式或者匿名类的实现，并且在这些实现中调用函数类型参数，那么这个高阶函数就不能被声明为内联函数。

为了应对这种情况，就有了crossinline这个关键字。它像一个契约，向编译器保证内联函数中的Lambda表达式中一定不会有return关键字，因此因return而起的冲突也就不存在了。
...
声明了crossinline之后，就无法在调用runRunable函数时的Lambda表达式中使用return关键字进行返回了。但是仍然可以使用return@runRunable的形式进行局部返回。除此之外，crossinline保留了内联函数的其他所有特性。
