...
这次的MyProvider类里，新增了4个整形变量，table1Dir表示访问table1表中的所有数据，table1Item表示访问表中的单条数据，table2Dir和table2Item同理。接着在MyProvider实例化的时候马上创建了UriMatcher的实例，并且调用addURI()方法，将期望匹配的内容URI格式传进去（此时传的路径参数是可以使用通配符的）。
