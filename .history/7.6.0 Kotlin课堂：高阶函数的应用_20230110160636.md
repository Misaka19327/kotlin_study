...
之后是这个Pair类型，正因为它是个键值对，所以需要通过泛型指定它的键和值都是什么类型。单对于ContentValues来说，因为ContentValues的键都是字符串类型，所以键的类型可以直接指定为String。但是ContentValues的值可以是任何类型的数据，甚至是null，所以这里的值的泛型指定为Any?。Any是kotlin里所有类的基类。

现在准备给这个cvOf()实现功能逻辑，思路就是先创建一个ContentValues对象
