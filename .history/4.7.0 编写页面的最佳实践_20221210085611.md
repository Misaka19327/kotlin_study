...
与以往的适配器类不同的是，这里能根据不同的viewType创建不同的界面。首先，定义了LeftViewHolder和RightViewHolder，分别用于缓存子项布局中的控件。然后重写getItemViewType()方法，使其返回当前position对应的消息类型。接下来在重写的onCreateViewHolder()中根据不同的
