...
接下来，借助UriMatcher这个类就可以实现匹配内容URI的功能。这个类里提供了一个addURI()方法，它接收3个参数，可以分别把authority，path和一个自定义代码传进去。这种情况下，当调用UriMatcher的match()方法时，就可以传进来一个Uri对象，返回值时某个能够匹配这个Uri对象所对应的自定义代码，利用这个代码，就可以判断对方想要访问什么数据了。
