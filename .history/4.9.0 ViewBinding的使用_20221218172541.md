...
能看到，在onBindViewHolder()中，每次都是通过findViewById()来获取实例。这样不仅没有达成这个插件原本的目的，连ViewHolder的作用都没有了。因为ViewHolder的本质作用是避免滑动列表时不停的调用findViewById()，但是目前这段代码的效果就是每次都会使用findViewById()。
