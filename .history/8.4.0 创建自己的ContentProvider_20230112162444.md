...
另外，如果要访问数据的话，被访问数据的应用必须处于运行状态。虽然目前来看好像通过ContentProvider去访问数据的话会自动启动被访问的应用，但是还是写一下，以防特殊情况下出问题。

官方说明如下：

[管理软件包可见性](https://developer.android.google.cn/training/basics/intents/package-visibility)
