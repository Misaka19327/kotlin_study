...
这段代码实际上，先创建了一个Job对象，然后把它传到CoroutineScope()方法中（注意这是个方法）。这个方法会返回一个CoroutineScope对象。借助这个对象就可以开启一个或者多个协程。这些借助CoroutineScope对象开启的协程，都会关联到Job对象的作用域下面，所以最后只需要调用Job对象的cancel()方法，就可以一次性结束这些
