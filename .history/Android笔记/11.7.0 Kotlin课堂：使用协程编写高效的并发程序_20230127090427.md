...
之后再看回这段代码，这个request()函数接收一个address参数。在它的内部，调用了之前说的suspendCoroutine()函数，它会立即挂起当前协程，然后在一个普通线程中执行Lambda表达式。在这个表达式中调用HttpUtil.sendHttpRequest()方法进行网络请求，并且用传统的回调方式监听请求结果。
