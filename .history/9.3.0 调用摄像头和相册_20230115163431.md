...
和之前startActivityForResult()不同，这次是调用了一个registerForActivityResult()来对Activity的结果进行监听。这个方法接收两个参数，一个是Contract类型，用于规定这个监听器的具体作用，这里传入的是ActivityResultContracts.StartActivityForResult。除此之外，
