...
最后，退出MainActivity时，执行了onPause()和onStop()，并最终销毁了Activity：
...
![1667217466712](image/3.4.0Activity的生命周期/1667217466712.png)
...
（另外，Bundle类型的变量也可以用来存储数据然后交给Intent，完成Activity之间的数据传输。未完成。代码应该没问题，但是在从intent里面取Bundle类型变量的时候，返回的是String类型，原因未知。bundle绝对是传过来了。用getStringExtra()能够知道
