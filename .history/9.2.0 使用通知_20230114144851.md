...
这里创建了一个Notification.BigTextStyle对象，它就是用来封装长文字信息的，只要调用它的bigText()方法把文字传进去就可以了。它可以允许通知显示很长一段文字而不被折叠，效果如下：（其实我怀疑显示不及预期这个锅要MIUI背，但是我没有证据）

![1673675687226](image/9.2.0使用通知/1673675687226.png)

除了长文字，借助setSytle()，通知也能显示图片：

![1673676418311](image/9.2.0使用通知/1673676418311.png)

和上面创建长文字是一样的步骤，这次创建的是NotificationCompat.BigPictureStyle对象。至于后面那串BitmapFactory，它是用来将图片转成Bitmap对象用的。效果如下（好吧我承认这确实不是MIUI的锅，正常都会折叠一下，只是怎么展开不太一样）：

![1673677459823](image/9.2.0使用通知/1673677459823.png)

###### 2 不同等级的不同效果

之前创建通知渠道的时候提到过，开发者能定义的消息重要程度只有三种。高重要程度的消息可以有声音和弹出横幅，中重要程度的消息只能出现在通知栏里，而低重要程度的消息在某些情况下会被隐藏和折叠。

这是高重要程度的消息：

![1673677926022](image/9.2.0使用通知/1673677926022.png)

效果：

![1673678410820](image/9.2.0使用通知/1673678410820.png)

先说结论，这样定义出的高重要程度的消息在Pixel上是没啥问题，有声音，有弹出横幅，下拉通知栏还会自动展开，但是在MIUI上不一样。在设置里看了一眼，这个Important的消息渠道是建立了没错，但是在通知方式上是都给改成了中优先程度，如果通知重复消息多了甚至会被直接归为不重要通知进行收纳：

![1673678902804](image/9.2.0使用通知/1673678902804.png)
