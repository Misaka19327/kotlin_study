...
第二个是Activity和Service绑定的。Activity调用Context类的bindService()方法来获取一个Service的连接时，就会调用Service的onBind()方法。如果Service在此前并没有被创建，那么onCreate()会先于onBind()执行。之后，Activity可以获取onBind()方法的返回的IBinder对象的实例，并通过这个对象自由调用Service的public方法
