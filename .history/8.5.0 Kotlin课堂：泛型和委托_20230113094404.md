...
首先，这里定义了一个Later类，并将其指定为泛型类。Later的构造函数中接受一个函数类型参数，这个函数类型参数不接收任何参数，并且返回值类型就是Later类指定的泛型。然后在这个类里实现getValue()方法，它的第一个参数指定为Any?类型，表示这个Later的委托功能在所有类中都可以使用。然后使用了一个value变量对值进行缓存，如果value为空就调用构造函数中传入的函数类型参数去获取值
