...
实际上，这段代码的核心就是by lazy{}。by表示委托属性，lazy则是一个高阶函数。在lazy函数中会创建一个Delegate对象，当调用uriMatcher时，其实调用的是Delegate对象的getValue()方法，然后在这个getValue()方法中又会调用lazy函数传入的Lambda表达式，这样表达式中的代码就能得到执行，并且调用p属性后得到的值就是Lambda表达式中最后一行代码
