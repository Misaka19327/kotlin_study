...
要跟系统请求权限，首先得查一下自己有没有这个权限，这一步使用的是ContextCompat.checkSelfPermission()方法，它接收两个参数，一个是上下文Context，另一个是具体的权限名。这里使用的是android.manifest.permission.CALL_PHONE（这里要注意，写的时候注意代码）。
