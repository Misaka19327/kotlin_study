...
这个模式和singleInstance很像，但是区别在于：singleInstance会启动一个专属的栈，并且这个栈有且只有这一个Activity。在整个应用中，这个Activity只有这一个实例。而singleInstancePerTask允许Activity和Intent.FLAG_ACTIVITY_MULTIPLE_TASK，Intent.FLAG_ACTIVITY_NEW_DOCUMENT相配合
