...
读取的方式基本上和存储的方式一致。forEachLine()是kotlin自带的一个扩展函数，它会将读到的每行内容都回调到Lambda表达式中，再在Lambda表达式中完成字符串的构架即可。

修改MainActivity的代码：

![1672992241145](image/7.2.0文件存储/1672992241145.png)
...
setSelection()方法用于将光标移动到指定位置。效果如下：

![1672992330807](image/7.2.0文件存储/1672992330807.png)
